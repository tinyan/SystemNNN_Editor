...
CScriptVoiceControl::CScriptVoiceControl(CMyApplicationBase* app,BOOL voicePack)
{
	m_app = app;
...
BOOL CScriptVoiceControl::LoadScriptVoice(LPSTR filename,int channel)
{
	if (m_readType)
	{
...
void CScriptVoiceControl::StopScriptVoice(int channel)
{
	m_voice[channel]->Stop();
}

void CScriptVoiceControl::PlayScriptVoice(int channel)
{
	m_voice[channel]->Play();
}

void CScriptVoiceControl::SetScriptVoiceVolume(int channel,int vol)
{
...
void CScriptVoiceControl::SetScriptVoiceTeii(int channel,int x,int y,int z)
{
	if (x>=128) x -= 256;
	if (y>=128) y -= 256;
	if (z>=128) z -= 256;
...
void CScriptVoiceControl::SetScriptVoiceMoveTime(int channel,int moveTime)
{
	m_voice[channel]->SetMoveTime(moveTime);
}

void CScriptVoiceControl::SetScriptVoiceTeii2(int channel,int x2,int y2,int z2)
{
	if (x2>=128) x2 -= 256;
	if (y2>=128) y2 -= 256;
	if (z2>=128) z2 -= 256;
...
void CScriptVoiceControl::SetScriptVoiceDoppler(int channel,BOOL dop)
{
	m_voice[channel]->SetDoppler(dop);
}


void CScriptVoiceControl::PlayScriptMessageVoice(CMessageData* pMessage,int channel)
{
