...
CChangeBGMList::CChangeBGMList(CMyApplicationBase* app)
{
	m_app = app;
...
CChangeBGMList::~CChangeBGMList()
{
	End();
}

void CChangeBGMList::End(void)
{
	DELETEARRAY(m_fromToTable);
	ENDDELETECLASS(m_bgmList2);
}

BOOL CChangeBGMList::AllChange(void)
{
	...
	m_bgmList2 = new CNameList();
	if (m_bgmList2->LoadFile("nnndir\\bgmlist2.txt") == FALSE)
	{
		ENDDELETECLASS(m_bgmList2);
		MessageBox(NULL,"nnndir\\bgmlist2.txtが見つかりません","Error",MB_OK | MB_ICONEXCLAMATION);
		return FALSE;
	}


	//make table
	m_bgmList = m_app->GetBGMList();
	int ks = m_bgmList->GetNameKosuu() / 2;
	int ks2 = m_bgmList2->GetNameKosuu() / 2;

	DELETEARRAY(m_fromToTable);
	m_fromToTable = new int[ks];
	
	int checkType = 0;
	int rt000 = MessageBox(NULL,"曲名で変換しますか(Noだとファイル名で変換)","選択",MB_YESNO);
	if (rt000 == IDYES)
	{
		checkType = 1;
	}
	...
	for (int i=0;i<ks;i++)
	{
		...
		int found = -1;
		LPSTR bgmName1 = m_bgmList->GetName(i*2+checkType);

		for (int ii=0;ii<ks2;ii++)
		{
			LPSTR bgmName2 = m_bgmList2->GetName(ii*2+checkType);
			if (_stricmp(bgmName1,bgmName2) == 0)
			{
				found = ii;
				break;
			}
...
//			CStoryBookData* pBook = pDoc2->GetStoryBookData();
	...
	//copy
	CopyFile("nnndir\\bgmlist2.txt","nnndir\\bgmlist.txt",FALSE);
	m_app->ReLoadBGMList();
...
BOOL CChangeBGMList::CheckRoutine(void)
{
//	CFilmCaseDoc* pDoc = (CFilmCaseDoc*)m_document[FILMCASE_WINDOW];
//	CFilmCaseData* pCase = pDoc->GetFilmCaseData();
	CFilmCaseData* pCase = m_app->GetFilmCaseData();
		...
		if (pFilm != NULL)
		{
			...
			for (int j=0;j<komaKosuu;j++)
			{
				...
				if (pKoma != NULL)
				{
					...
					if (bgm > 0)
					{
						if (m_fromToTable[bgm] == -1)
						{
							PrintError(m_app->GetFileNameOnly(),pFilm->GetMyName(),j);
							return FALSE;
						}
					}
				}
...
void CChangeBGMList::PrintError(LPSTR nnnName,LPSTR filmName,int koma)
{
	char emes[1024];
	wsprintf(emes,"%s\n%s\n%dコマ\n%d番",nnnName,filmName,koma);
	MessageBox(NULL,emes,"変換できない曲エラー",MB_OK | MB_ICONEXCLAMATION);
}


BOOL CChangeBGMList::ChangeBGMRoutine(void)
{
//	CFilmCaseDoc* pDoc = (CFilmCaseDoc*)m_document[FILMCASE_WINDOW];
//	CFilmCaseData* pCase = pDoc->GetFilmCaseData();
	CFilmCaseData* pCase = m_app->GetFilmCaseData();
		...
		if (pFilm != NULL)
		{
			...
			for (int j=0;j<komaKosuu;j++)
			{
				...
				if (pKoma != NULL)
				{
					...
					if (bgm > 0)
					{
						pKoma->SetBGMNumber(m_fromToTable[bgm]);
					}
				}
			}
		}
...
/*
BOOL CChangeBGMList::ChangeBGMList(BOOL flg)
{
//	CFilmCaseDoc* pDoc = (CFilmCaseDoc*)m_document[FILMCASE_WINDOW];
//	CFilmCaseData* pCase = pDoc->GetFilmCaseData();
	CFilmCaseData* pCase = m_app->GetFilmCaseData();
