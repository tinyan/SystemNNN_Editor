...
CScriptSoundControl::CScriptSoundControl(CMyApplicationBase* app)
{
	m_app = app;
...
BOOL CScriptSoundControl::LoadScriptSound(int nm,int channel)
{
...
void CScriptSoundControl::StopScriptSound(int channel)
{
	m_sound[channel]->Stop();
}

void CScriptSoundControl::PlayScriptSound(int channel,BOOL loopFlag)
{
	m_sound[channel]->Play(loopFlag);
}

void CScriptSoundControl::SetScriptSoundVolume(int channel,int vol)
{
...
void CScriptSoundControl::SetScriptSoundTeii(int channel,int x,int y,int z)
{
	if (x>=128) x -= 256;
	if (y>=128) y -= 256;
	if (z>=128) z -= 256;
...
void CScriptSoundControl::SetScriptSoundMoveTime(int channel,int moveTime)
{
	m_sound[channel]->SetMoveTime(moveTime);
}

void CScriptSoundControl::SetScriptSoundTeii2(int channel,int x2,int y2,int z2)
{
	if (x2>=128) x2 -= 256;
	if (y2>=128) y2 -= 256;
	if (z2>=128) z2 -= 256;
...
void CScriptSoundControl::SetScriptSoundDoppler(int channel,BOOL dop)
{
	m_sound[channel]->SetDoppler(dop);
}

void CScriptSoundControl::PlaySeTest(int n,int channel)
{
	if (n <= 0) return;
...
void CScriptSoundControl::PlayScriptMessageSound(CMessageData* pMessage,int channel)
{
	//‚³‚¢‚¿‚¥‚Á‚­
